-- ============================================
--  NINJA HUB - FULL PACKAGE V4.3
--  Tác giả: Hoàng (cập nhật bởi Noguchi Hyuga)
--  [GHÉP HOÀN CHỈNH + DANH SÁCH SERVER + HOP KHI BẤM]
-- ============================================
...
-- ============================================
-- BIẾN TOÀN CỤC ĐỂ QUẢN LÝ UI
-- ============================================
local HopHistory = {}  -- Lưu các server đã hop để không hiển thị lại
...
-- Hop đến server và xóa icon
local function hopToServer(jobId)
    if not jobId or jobId == game.JobId then 
        print("❌ Đang ở server này rồi!")
        return false 
    end
    
    -- Lưu server vào lịch sử để xóa icon
    HopHistory[jobId] = true
    
    pcall(function()
        TeleportService:TeleportToServerInstance(game.PlaceId, jobId, nil)
        print("🚀 Đang hop đến server:", jobId)
        
        -- Xóa khỏi lịch sử sau 5 phút (để có thể hiển thị lại nếu cần)
        task.delay(300, function()
            HopHistory[jobId] = nil
        end)
    end)
    return true
end
    ...
    pcall(function()
        ...
        for _, server in pairs(result) do
            if server and server.JobId then
                -- Chỉ thêm server nếu chưa có trong lịch sử hop
                if not HopHistory[server.JobId] then
                    table.insert(servers, {
                        jobId = server.JobId,
                        players = server.PlayerCount or 0,
                        ping = server.Ping or 0,
                        isCurrent = server.JobId == game.JobId
                    })
                end
            end
        end
...
-- KIỂM TRA BOSS TRONG SERVER HIỆN TẠI
local function checkBossInCurrentServer(bossName)
    local found = false
    pcall(function()
        if Enemies and Enemies:FindFirstChild(bossName) then found = true end
        if RS and RS:FindFirstChild(bossName) then found = true end
        if Workspace and Workspace:FindFirstChild(bossName) then found = true end
    end)
    return found
end

-- LẤY DANH SÁCH SERVER CÓ BOSS (KHÔNG HIỂN THỊ ICON ĐÃ HOP)
local function getServersWithBoss(bossName)
    local servers = getServerList()
    local result = {}
    
    for _, server in pairs(servers) do
        -- Bỏ qua server đã hop
        if HopHistory[server.jobId] then
            print("⏳ Bỏ qua server đã hop:", server.jobId)
        else
            local hasBoss = false
            if server.isCurrent then
                hasBoss = checkBossInCurrentServer(bossName)
            end
            table.insert(result, {
                jobId = server.jobId,
                ...
                status = server.isCurrent and (hasBoss and "🟢 Có boss" or "🔴 Không có boss") or "❓ Chưa xác định"
            })
        end
    end
...
getgenv().NinjaHopCore = {
    ...
    getSea = getSea,
    getServersWithBoss = getServersWithBoss,
    HopHistory = HopHistory,
}

-- ============================================
-- PHẦN 2: UI MODULE (Cập nhật với nút hop)
-- ============================================
task.spawn(function()
    task.wait(1)
    ...
    local function createUI()
        ...
        local window = library:CreateWindow({
            Title = '🔥 NINJA HUB V4.3',
            Author = 'Hoàng (Noguchi)',
            Size = UDim2.fromOffset(800, 700),
            Transparent = true,
        })

        -- ============================================
        -- HÀM TẠO NÚT HOP CHO TỪNG SERVER
        -- ============================================
        local function createHopButton(parentTab, bossName)
            local core = getgenv().NinjaHopCore
            if not core then return end
            
            local servers = core.getServersWithBoss(bossName)
            
            for _, sv in pairs(servers) do
                -- Chỉ hiển thị server chưa hop
                if not core.HopHistory[sv.jobId] then
                    local currentTag = sv.isCurrent and " 👈 Hiện tại" or ""
                    local buttonText = string.format("📡 Server: %s | Người chơi: %d | Ping: %dms | %s%s", 
                        string.sub(sv.jobId, 1, 12).."...", sv.players, sv.ping or 0, sv.status, currentTag)
                    
                    parentTab:Button({
                        Title = buttonText,
                        Callback = function()
                            print("🚀 Đang hop đến server:", sv.jobId)
                            core.hopToServer(sv.jobId)
        ...
        -- TAB TỔNG QUAN
        -- ============================================
        local mainTab = window:Tab({Title = '🏠 Tổng quan'})
        
        mainTab:Paragraph({
            Title = '📌 NINJA HUB V4.3',
            Desc = 'Tác giả: Hoàng (cập nhật bởi Noguchi Hyuga)\nTrạng thái: 🟢 Đang hoạt động'
        })
        ...
        -- TAB DOUGH KING
        -- ============================================
        local doughTab = window:Tab({Title = '🍩 Dough King'})
        
        doughTab:Paragraph({
            Title = '🍩 DOUGH KING',
            Desc = 'Danh sách server có Dough King - Bấm vào server để hop'
        })
        
        -- Tạo nút hop cho từng server
        createHopButton(doughTab, "Dough King")
        
        doughTab:Button({
            Title = '🔄 Làm mới danh sách',
            Callback = function()
                print("🔍 Đang tìm server có Dough King...")
                local core = getgenv().NinjaHopCore
                if core and core.getServersWithBoss then
                    local servers = core.getServersWithBoss("Dough King")
                    if #servers > 0 then
                        for _, sv in pairs(servers) do
                            if not core.HopHistory[sv.jobId] then
                                print(string.format("📡 Server: %s | Người chơi: %d | Ping: %dms | %s", 
                                    sv.jobId, sv.players, sv.ping or 0, sv.status))
                            end
                        end
                    else
                        print("❌ Không tìm thấy server nào hoặc đã hop hết")
                    end
                end
            end
        })
        ...
        -- TAB DARKBEARD
        -- ============================================
        local darkTab = window:Tab({Title = '🏴 Darkbeard'})
        
        darkTab:Paragraph({
            Title = '🏴 DARKBEARD',
            Desc = 'Danh sách server có Darkbeard - Bấm vào server để hop'
        })
        
        createHopButton(darkTab, "Darkbeard")
        
        darkTab:Button({
            Title = '🔄 Làm mới danh sách',
            Callback = function()
                print("🔍 Đang tìm server có Darkbeard...")
                local core = getgenv().NinjaHopCore
                if core and core.getServersWithBoss then
                    local servers = core.getServersWithBoss("Darkbeard")
                    for _, sv in pairs(servers) do
                        if not core.HopHistory[sv.jobId] then
                            print(string.format("📡 Server: %s | Người chơi: %d | Ping: %dms | %s", 
                                sv.jobId, sv.players, sv.ping or 0, sv.status))
        ...
        -- TAB SOUL REAPER
        -- ============================================
        local soulTab = window:Tab({Title = '👻 Soul Reaper'})
        
        soulTab:Paragraph({
            Title = '👻 SOUL REAPER',
            Desc = 'Danh sách server có Soul Reaper - Bấm vào server để hop'
        })
        
        createHopButton(soulTab, "Soul Reaper")
        
        soulTab:Button({
            Title = '🔄 Làm mới danh sách',
            Callback = function()
                print("🔍 Đang tìm server có Soul Reaper...")
                local core = getgenv().NinjaHopCore
                if core and core.getServersWithBoss then
                    local servers = core.getServersWithBoss("Soul Reaper")
                    for _, sv in pairs(servers) do
                        if not core.HopHistory[sv.jobId] then
                            print(string.format("📡 Server: %s | Người chơi: %d | Ping: %dms | %s", 
                                sv.jobId, sv.players, sv.ping or 0, sv.status))
        ...
        -- TAB RIP INDRA
        -- ============================================
        local ripTab = window:Tab({Title = '⚡ Rip Indra'})
        
        ripTab:Paragraph({
            Title = '⚡ RIP INDRA',
            Desc = 'Danh sách server có Rip Indra - Bấm vào server để hop'
        })
        
        createHopButton(ripTab, "Rip Indra")
        
        ripTab:Button({
            Title = '🔄 Làm mới danh sách',
            Callback = function()
                print("🔍 Đang tìm server có Rip Indra...")
                local core = getgenv().NinjaHopCore
                if core and core.getServersWithBoss then
                    local servers = core.getServersWithBoss("Rip Indra")
                    for _, sv in pairs(servers) do
                        if not core.HopHistory[sv.jobId] then
                            print(string.format("📡 Server: %s | Người chơi: %d | Ping: %dms | %s", 
                                sv.jobId, sv.players, sv.ping or 0, sv.status))
        ...
        -- TAB CURSED CAPTAIN
        -- ============================================
        local cursedTab = window:Tab({Title = '⚓ Cursed Captain'})
        
        cursedTab:Paragraph({
            Title = '⚓ CURSED CAPTAIN',
            Desc = 'Danh sách server có Cursed Captain - Bấm vào server để hop'
        })
        
        createHopButton(cursedTab, "Cursed Captain")
        
        cursedTab:Button({
            Title = '🔄 Làm mới danh sách',
            Callback = function()
                print("🔍 Đang tìm server có Cursed Captain...")
                local core = getgenv().NinjaHopCore
                if core and core.getServersWithBoss then
                    local servers = core.getServersWithBoss("Cursed Captain")
                    for _, sv in pairs(servers) do
                        if not core.HopHistory[sv.jobId] then
                            print(string.format("📡 Server: %s | Người chơi: %d | Ping: %dms | %s", 
                                sv.jobId, sv.players, sv.ping or 0, sv.status))
        ...
        -- TAB TẤT CẢ SERVER
        -- ============================================
        local allTab = window:Tab({Title = '📊 All Servers'})
        
        allTab:Paragraph({
            Title = '📊 DANH SÁCH TẤT CẢ SERVER',
            Desc = 'Hiển thị tất cả server đang có (không xóa icon)'
        })
        
        allTab:Button({
            Title = '🔄 Làm mới danh sách',
            Callback = function()
                print("📊 DANH SÁCH SERVER:")
                local core = getgenv().NinjaHopCore
                if core and core.getServerList then
                    local servers = core.getServerList()
                    for _, sv in pairs(servers) do
                        local status = sv.isCurrent and "👈 Hiện tại" or ""
                        print(string.format("📡 ID: %s | Người chơi: %d | Ping: %dms %s", 
                            sv.jobId, sv.players, sv.ping or 0, status))
                    end
                end
            end
        })
        ...
        allTab:Button({
            Title = '🚀 Hop đến server ít người nhất',
            Callback = function()
                local core = getgenv().NinjaHopCore
                if core and core.getServerList and core.hopToServer then
                    local servers = core.getServerList()
                    local bestServer = nil
                    local minPlayers = math.huge
                    for _, server in pairs(servers) do
                        if not server.isCurrent and server.players < minPlayers then
                            minPlayers = server.players
                            bestServer = server
                        end
                    end
                    if bestServer then
                        print(string.format("🚀 Hop đến server: %s (Người chơi: %d)", bestServer.jobId, bestServer.players))
                        core.hopToServer(bestServer.jobId)
                    else
        ...
        islandTab:Paragraph({
            Title = '🏝️ AUTO HOP ISLAND',
            Desc = 'Tự động hop đến server có đảo đã chọn'
        })
        ...
        chestTab:Paragraph({
            ...
            Desc = 'Tự động hop khi có drop từ rương'
        })
        ...
        otherTab:Paragraph({
            Title = '⚔️ CÁC LOẠI HOP KHÁC',
            Desc = 'Elite, Dealer, Fruit, Event'
        })
        ...
        infoTab:Paragraph({
            Title = '📖 NINJA HUB V4.3',
            Desc = 'Version: 4.3\nTác giả: Hoàng (cập nhật bởi Noguchi Hyuga)\n\n✅ Hỗ trợ hop:\n- Boss (8 loại)\n- Elite (3 loại)\n- Đảo (4 loại)\n- Dealer (2 loại)\n- Fruit\n- Holy Grail (5%)\n- Darkbeard Key (3%)\n- Full Moon\n- Pirate Raid\n- Factory Raid\n- Berry\n- Rare NPC\n\n📊 Mới: Bấm vào server để hop\n🗑️ Xóa icon sau khi hop'
        })
        ...
        infoTab:Button({
            Title = '🔄 Reset lịch sử hop',
            Callback = function()
                local core = getgenv().NinjaHopCore
                if core and core.HopHistory then
                    for key, _ in pairs(core.HopHistory) do
                        core.HopHistory[key] = nil
                    end
                    print("✅ Đã reset lịch sử hop, các server sẽ hiển thị lại!")
                end
            end
        })
    end
...
print("✅ NINJA HUB FULL PACKAGE V4.3 loaded!")
print("📌 Bật/tắt các chức năng qua UI!")
print("📊 Bấm vào server trong tab boss để hop và xóa icon!")